...
class Lunar {
    ...
    /**
     * [getLunarYearMonths 获取农历每年的月数]
     * @param  [year] $year [农历年份]
    ...
    /**
     * [getFestival 获取节日]
     * @param  boolean $Year  [年]
     * @param  boolean $Month [月]
     * @param  boolean $day   [日]
     * @return [array]         [是返回节日信息/否array('is'=>false)]
     */
    public function getFestival($Year = false,$Month = false,$day = false){
        $festivals=array(
            array(
                array(
                    "date"=>"正月初一",
     ...
     */    
   public function getFestival_two($today, $nl_info = false,$config = 1)    
    {    
        if($config == 1)    
        {    
