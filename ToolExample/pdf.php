<?php 
/**
 * A Example For Make A PDF File
 */
require '../vendor/autoload.php';
// 新建一个pdf文件：
// Orientation：文档打印格式Landscape为横式打印，Portrait为纵向打印 
...
/*设置页面边幅：Left：左边幅 Top：顶部边幅 Right：右边幅  Keepmargins：为true时，覆盖默认的PDF边幅。 */
$pdf->SetMargins(PDF_MARGIN_LEFT, PDF_MARGIN_TOP,PDF_MARGIN_RIGHT);

/*设置单元格的边距：Left：左边距 Top：顶部边距 Right：右边距 Bottom：底部边距。*/
...
/*设置线条的风格：Width：设置线条粗细 Cap：设置线条的两端形状 Join：设置线条连接的形状
Dash：设置虚线模式 Color：设置线条颜色，一般设置为黑色，如：array(0, 0, 0)。*/
$pdf->SetLineStyle(array('width' => 0.2, 'cap' => 'butt', 'join' => 'miter', 'dash' => '0', 'color' => array(0, 0,0)));

// 画一条线： x1：线条起点x坐标 y1：线条起点y坐标 x2：线条终点x坐标 y2：线条终点y坐标 style：SetLineStyle的效果一样
$pdf->Line($x1, $y1, $x2, $y2, $style=array());

/*执行一个换行符，横坐标自动移动到左边距的距离，纵坐标换到下一行：H：设置下行跟上一行的距离，默认的话，高度为最后一个单元格的高度 Cell：true，添加左或右或上的间距到横坐标。 */
$pdf->Ln($h='', $cell=false);
...
Format：设置此页面的打印格式。Keepmargins：true，以当前的边幅代替默认边幅来重写页面边幅。
