<?php 
$number = array("1390379","139379","1350388","138379","138388","158379","1571379","1362379","137831","152361","151363","150379","188388","188489","187363","184385","184388","1883798","188379","188388","1380388","1360388","1361379","1361388","1360866","1360865","188379","187379");
// $number = array("1390379","139379","1350388");
ini_set('memory_limit','-1');
set_time_limit(0);

function demo($number,$num = 100000)
{
    ...
    for ($i=0; $i < $num; $i++) { 
        $needle = $i+100000;
        $seven_needle = substr($needle, 2);
        $six_needle = substr($needle, 1);
        foreach ($number as $v) {
            if(strlen($v)===7){
                $arr['number']=$v.$seven_needle;
                $arr['name']=$name->getName(2);
                array_push($newarr, $arr);
            }
            else{
                $arr['number']=$v.$six_needle;
                $arr['name']=$name->getName(2);
                array_push($newarr, $arr);
            }
        }
    ...
    $mysql = new MySQLi;
    // var_dump(get_class_methods($mysql));exit;
    $mysql->connect('127.0.0.1','root','');
    $mysql->select_db('test');
    $mysql->set_charset('utf8');
    $newarr = array_chunk($newarr, $num%250+1);
    foreach ($newarr as $v) {
        sort($v);
        if (insert($v,count($v),$mysql)) {
...
function insert(&$newarr,$num,$mysql)
{
    if (!$num) {
        // echo $num;
        return 1;
    ...
    // echo 'INSERT INTO user (`name`,`number`) VALUES ("'.$newarr[$index]['name'].'","'.$newarr[$index]['number'].'")';exit;
    $mysql->query('BEGIN');
    $con =$mysql->query('INSERT INTO user_copy (`name`,`number`) VALUES ("'.$newarr[$index]['name'].'","'.$newarr[$index]['number'].'")');
    if ($num%50 == 0) {
        $mysql->query('COMMIT');
        $mysql->query('BEGIN');
        // echo $num,'<br>';
    ...
    if($con){
        unset($newarr[$index]);
        sort($newarr);
        insert($newarr,$num-1,$mysql);
    }
    else{
        insert($newarr,$num,$mysql);
    }
}
...
echo rand_string($len=6,$type=4,$addChars='');exit;
/**
 * 生成随机字符串
